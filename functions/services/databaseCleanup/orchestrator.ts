...
import { enrichReleaseDates } from "./enrichReleaseDates";
import { consolidateRedundantFields } from "./consolidateRedundantFields";
import { normalizeTracking } from "./normalizeTracking";
import { validateTvProgress } from "./validateTvProgress";

interface CleanupReport {
  executedAt: string;
  userId: string;
  phase1_analysis: any;
  phase2_enrichment?: any;
  phase3_consolidation?: any;
  phase4_normalization?: any;
  phase5_tvProgress?: any;
 ...
 */
export const cleanupLibraryDatabase = functions.https.onCall(
  async (data: any, context: any) => {
    try {
      ...
      try {
        ...
        report.phase1_analysis = await analyzeDatabase(userId);
      ...
      if (!skipPhases.includes(2)) {
        try {
          ...
          report.phase2_enrichment = await enrichReleaseDates(userId);
      ...
      if (!skipPhases.includes(3)) {
        try {
          ...
          report.phase3_consolidation = await consolidateRedundantFields(userId);
      ...
      if (!skipPhases.includes(4)) {
        try {
          ...
          report.phase4_normalization = await normalizeTracking(userId);
      ...
      if (!skipPhases.includes(5)) {
        try {
          ...
          report.phase5_tvProgress = await validateTvProgress(userId);
 ...
 */
export const previewDatabaseCleanup = functions.https.onCall(
  async (_data: any, context: any) => {
    ...
    const report = await analyzeDatabase(context.auth.uid);
